...
using Tatweer.SafeCity.Authorization.Accounts;
...
using Tatweer.SafeCity.Departments;
using System.Text;
...
namespace Tatweer.SafeCity.Users
{
    ...
    public class UserAppService : AsyncCrudAppService<User, UserDto, long, PagedUserResultRequestDto, CreateUserDto, UserDto>, IUserAppService
    {
        ...
        public UserAppService(
            ...
            IRepository<SafeCityAttachment> attachmentReposioty
            )
            : base(repository)
        {
        ...
        public async Task<HttpStatusCode> ResetPasswordRequest(ResetPasswordRequestDto input)
        {
            try
            {
                ...
                try 
                {
                    ...
                    //PostmarkClient client = new PostmarkClient("92cdd22a-b5a5-48f3-9d8d-c488213c2efc");
                    //PostmarkResponse response =await client.SendMessageAsync(postmarkMessage);

                    //if (response.Status != PostmarkStatus.Success)
                    //{
                    //    throw new UserFriendlyException("Response was: " + response.Message);
                    //}

                  //  await _emailManager.SendMail(user.EmailAddress, body.ToString(), "Password Reset");
                }
                catch (Exception e)
                {
        ...
        public async Task<UserDto> GetUserById(long id)
        {
            ...
            var userDto = this.MapToEntityDto(user);
        ...
        public override async Task<UserDto> GetAsync(EntityDto<long> input)
        {
            try
            {
                ...
                var userProfileAttachment = await _attachmentReposioty.GetAll().Where(x => x.EntityId == userDto.Id && x.AttachmentType == SafeCityAttachmentsEnum.UserProfile.ToString()).OrderByDescending(x => x.Id).Select(x => new SafeCityAttachmentDto
                {
                    Id = x.Id,
                    EntityId = x.EntityId,
        ...
        public override async Task<PagedResultDto<UserDto>> GetAllAsync(PagedUserResultRequestDto input)
        {
            ...
            //{
            //    throw new System.Exception("User department mapping doesn't exists");
            //}
            else
            {
