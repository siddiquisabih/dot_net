    ...
    public class NotificationAppService : INotificationAppService
    {
        ...
        public async Task SaveTaskGroup(NotificationGroupDto notificationGroupDto)
        {
            ...
            using (var unitOfWork = _unitOfWorkManager.Begin(TransactionScopeOption.Suppress))
            {
                ...
                //{
                //    await _taskHub.Groups.AddToGroupAsync(notificationGroupDto.GroupName, userConnection.ConnectionId);
                //}
                unitOfWork.Complete();
            }
        }
        ...
        public async Task<bool> UpdateNotificationGroup(NotificationGroupDto notificationGroupDto)
        {
            ...
            if (notificationGroupDto != null)
            {
                ...
                using (var unitOfWork = _unitOfWorkManager.Begin(TransactionScopeOption.Suppress))
                {
                    ...
                    //var userConnections = _userSignalrConnectionRepository
                    //    .GetAllList(us => notificationGroupDto.AssigneeIds.Contains(Convert.ToInt32(us.UserId)))
                    //    .Distinct().ToList();
                   
                    //var alreadyExistUserConnections= _userSignalrConnectionRepository
                    //    .GetAllList(us => AlreadyExistUserNotificationGroupEntityList.Select(y=>y.UserId)
                    //    .Contains(Convert.ToInt32(us.UserId)))
                    //    .Distinct().ToList();
                    
                    //var leaveGroupUserConnections= alreadyExistUserConnections
                    //    .Except(userConnections).ToList();

                    //foreach (var leaveGroupUserConnection in leaveGroupUserConnections)
                    //{
                    //    await _taskHub.Groups.RemoveFromGroupAsync(notificationGroupDto.GroupName, leaveGroupUserConnection.ConnectionId);
                    //}
